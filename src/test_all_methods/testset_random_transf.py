...
def main_dips_het():
    raw_data_path = './data/dips-het-test-100/interim/pairs-pruned/'
    split_files_path = './data/dips-het-test-100/test_100.txt'
    final_dir = './test_sets_pdb/dips_het_test_random_transformed/'
    ...
    for dill_filename in selected_files:
        ...
        ppdb0 = PandasPdb()
        ppdb0._df['ATOM'] = dips_het_update_df(df0)
        ppdb0._df['ATOM'] = ppdb0._df['ATOM'][cols]

        ppdb1 = PandasPdb()
        ppdb1._df['ATOM'] = dips_het_update_df(df1)
        ppdb1._df['ATOM'] = ppdb1._df['ATOM'][cols]

        complex_dir = os.path.join(final_dir, 'complexes') 
        random_dir = os.path.join(final_dir, 'random_transformed')
        ...
        random_transf_pdb(ppdb0, os.path.join(complex_dir, dill_filename.replace('/', '_') + '_l_b_COMPLEX.pdb'), unchanged=True)
        random_transf_pdb(ppdb1, os.path.join(complex_dir, dill_filename.replace('/', '_') + '_r_b_COMPLEX.pdb'), unchanged=True)

        random_transf_pdb(ppdb0, os.path.join(random_dir, dill_filename.replace('/', '_') + '_l_b.pdb'))
        random_transf_pdb(ppdb1, os.path.join(random_dir, dill_filename.replace('/', '_') + '_r_b.pdb'))



def main_antibody_antigen():
    raw_data_path = './data/antibody-antigen-test-50/interim/pairs-pruned/'
    split_files_path = './data/antibody-antigen-test-50/test_50.txt'
    final_dir = './test_sets_pdb/antibody_antigen_test_random_transformed/'
