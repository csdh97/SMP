...
class Unbound_Bound_Data(Dataset):
    ...
    def __init__(self, args, if_swap=True, reload_mode='train',
        ...
        frac_str = ''
        if reload_mode == 'train' and args['data'] == 'dips_het':
        ...
        if load_from_cache:
            ...
            #     length = len(self.pocket_coors_list)
            #     sample_indices = random.sample(range(length), math.floor(length * data_fraction))

            #     self.pocket_coors_list = [self.pocket_coors_list[i] for i in sample_indices]
            #     self.bound_ligand_repres_nodes_loc_array_list = [self.bound_ligand_repres_nodes_loc_array_list[i] for i in sample_indices]
            #     self.bound_receptor_repres_nodes_loc_array_list = [self.bound_receptor_repres_nodes_loc_array_list[i] for i in sample_indices]
            #     self.ligand_graph_list = [self.ligand_graph_list[i] for i in sample_indices]
            #     self.receptor_graph_list = [self.receptor_graph_list[i] for i in sample_indices]
        else:
