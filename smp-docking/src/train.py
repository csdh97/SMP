...
def save_checkpoint(filename, model, optimizer, args, epoch):
    ...
    non_load_keys = ['device', 'debug', 'worker', 'n_jobs']
...
def main(args):
    ...
    if args['method'] == 'smp' and args['resume_ckpt'] is not None:
        print('loading pretrained model')
        ckpt = torch.load(args['resume_ckpt'], map_location=torch.device('cpu'))
    ...
    model = train(args, tb_logger, train_loader, val_loader, test_loader, model, nn.MSELoss(reduction='mean'))
...
def train(args, tb_logger, train_loader, val_loader, test_loader, model, loss_fn_coors):
    ...
    for epoch in range(args['num_epochs']):
        ...
        # if val_complex_rmsd_median < best_val_rmsd_median:
    ...
    save_checkpoint(os.path.join(args['checkpoint_dir'], args['data'] + '_model_latest.pth'), model, optimizer, args, epoch)
